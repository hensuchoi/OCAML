 ...
 * e.g. (removeDuplicates [1;6;2;4;(* CSE 130: Programming Assignment 2 *)
 ...
 *   that ki = k.  If such a ki is found, then vi is returned.  Otherwise, if no such
 *   ki exists in the list, d is returned.
 * e.g. (assoc (-1,"william",[("ran(* CSE 130: Programming Assignment 2 *)
 ...
 *   ki exists in the list, d is returned.
 * e.g. (assoc (-1,"william",[("ranjit",85);("william",23);("moose",44)]))
 *        returns 23(* CSE 130: Programming Assignment 2 *)
 ...
 * third ... occurrences) removed, and where the remaining elements 
 * appear in the same order as in l.
 * e.g. (removeDuplicates [1;6;2;4;jit",85);("william",23);("moose",44)]))
 *        returns 23
 *      (assoc (-1,"bob",[("ranjit",85);("william",23);("moose",44)]))
 *        returns -1
 *
 ...
 *)
   ...
   let rec assoc (d,k,l) = match  l with
   | []    -> d12;2;13;6;9]) is [1;6;2;4;12;13;9]
 *
 ...
 *)
   ...
   let removeDuplicates l = (* CSE 130: Programming Assignment 2 *)
 ...
 *)
   ...
   let fixpoint (f,b) = wwhile ((fun (b : 'a) -> ((f b), f b != b)),b);;
 ...
 *)
   ...
   | Log      e1            -> "log" ^ "(" ^ "abs(" ^ exprToString e1 ^ "))"
   | Average  (e1,e2)       -> "(" ^ "(" ^ exprToString e1 ^ "+" ^ exprToString e2 ^ ")/2)"
   | Times    (e1, e2)      -> exprToString e1 ^ "*"^ exprToString e2 
   | Magic    (e1,e2,e3)    -> "sqrt abs(" ^ exprToString e1 ^ ")*sqrt abs("^ exprToString e2 ^ ")*sqrt abs(" ^ exprToString e3 ^")"
 ...
 *)
   ...
   let buildMagic(e1,e2,e3)           = Magic (e1,e2,e3)
   ...
   | VarX -> x
   | VarY -> y
   | Sine e2 -> sin (pi *. eval (e2,x,y))
   | Cosine (e1) -> cos (pi *. eval (e1,x,y))
   | Log (e1) -> sqrt(abs_float(eval (e1,x,y)))
   | Average (e1,e2)->  (eval (e1,x,y) +. eval (e2,x,y)) /. 2.0
   | Magic (e1,e2,e3) -> sqrt(abs_float (sin(eval (e1,x,y)))) *. sqrt(abs_float (cos(eval (e2,x,y)))) *. sqrt(abs_float (eval (e3,x,y))) 
 ...
 *)
   ...
   | 2->   buildThresh(build (rand, depth-1),build (rand, depth-1),build (rand, depth-1),build (rand, depth-1))
   | 3 ->  buildLog(build (rand,depth -1))
   | 4 ->  buildAverage(build (rand, depth-1),build (rand, depth-1))            
   | 5 ->  buildTimes(build (rand, depth-1),build (rand, depth-1))
   | 6 ->  buildMagic (build (rand, depth-1),build (rand, depth-1),build (rand, depth-1))
